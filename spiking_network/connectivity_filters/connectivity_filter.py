...
class ConnectivityFilter(AbstractConnectivityFilter):
    ...
    def time_dependence(self, W0, i, j):
        ...
        t = torch.arange(self.time_scale).repeat(W0.shape[0], 1)
        is_self_edge = (i==j).unsqueeze(1).repeat(1, self.time_scale)
        self_edges = self.abs_ref_strength * (t < self.abs_ref_scale) + self.rel_ref_strength * torch.exp(-self.beta * (t - self.abs_ref_scale)) * (self.abs_ref_scale <= t) * (t <= self.abs_ref_scale + self.rel_ref_scale)
        other_edges = torch.einsum("i, ij -> ij", W0, torch.exp(-self.alpha * t) * (t < self.influence_scale))
