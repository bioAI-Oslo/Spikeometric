...
def save_parallel(
    ...
    for i, (x_sim, W_sim, edge_index_sim) in enumerate(zip(x_sims, Ws, edge_indices)):
        ...
        np.savez(
            ...
            X_sparse=sparse_x,
            W=W_sim,
            w_0=connectivity_filter.W0[i*cluster_size:(i+1)*cluster_size, i*cluster_size:cluster_size*(i+1)],
...
def save(spikes, connectivity_filter, n_steps, seed, data_path):
    ...
    sparse_x = coo_matrix(
        (data, (x, t)), shape=(connectivity_filter.W0.shape[0], n_steps)
    )
    np.savez_compressed(
        data_path,
        X_sparse=sparse_x,
...
def make_dataset(
    n_clusters, cluster_size, n_cluster_connections, n_steps, n_datasets, data_path
):
    """Generates a dataset"""
    # Set data path
    data_path = (
    ...
    with torch.no_grad():  # Disables gradient computation (the models are built on top of torch)
        for i in tqdm(range(n_datasets)):
            ...
            save_parallel(
                ...
                i,
                data_path,
            )  # Saves the spikes and the connectiv 7ity filter to a file
