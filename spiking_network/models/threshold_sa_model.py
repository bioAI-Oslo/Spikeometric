...
class ThresholdSAM(SAModel):
    ...
    and :math:`f_i(t+1)` is the stimulation input of neuron :math:`i` at time :math:`t+1`.
    ...
    def input(self, edge_index: torch.Tensor, W: torch.Tensor, state: torch.Tensor, t=-1, stimulus_mask=False) -> torch.Tensor:
        ...
        and :math:`f(t)` is the stimulation input at time :math:`t`.
        ...
        """
        return self.r*self.synaptic_input(edge_index, W, state=state) + self.background_input(state.shape[0]) + self.stimulation_input(t, stimulus_mask)
