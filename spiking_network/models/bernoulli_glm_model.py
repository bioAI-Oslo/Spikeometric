...
class BernoulliGLM(BaseModel):
    ...
    def __init__(self, 
        ...
        T_ms = max(coupling_window, abs_ref_scale + rel_ref_scale) # This is the max number of time steps we need to consider for the coupling window and the refractory periods
        T = int(T_ms / dt) # This is the number of time steps we need to consider for the coupling window and the refractory periods
    ...
    def input(self, edge_index: torch.Tensor, W: torch.Tensor, state: torch.Tensor, t=-1, stimulus_mask: torch.Tensor = 0) -> torch.Tensor:
        r"""
        Computes the input at time step :obj:`t+1` by adding the synaptic input from neighboring neurons and the stimulation input.
        ...
        """
        return self.synaptic_input(edge_index, W, state=state) + self.stimulation_input(t, stimulus_mask)
    ...
    def connectivity_filter(self, W0: torch.Tensor, edge_index: torch.Tensor) -> torch.Tensor:
        ...
        t = t.repeat(W0.shape[0], 1) # [n_edges, T]

        # Boolean tensor indicating whether the edge is a self-edge
        is_self_edge = (i==j).unsqueeze(1).repeat(1, self.T)
        ...
        refractory_edges = abs_ref + rel_ref
        normal_edges = torch.einsum("i, ij -> ij", W0, torch.exp(-torch.abs(self.alpha)*self.dt * t)*(t < self.coupling_window))

        W = torch.where(is_self_edge, refractory_edges, normal_edges).flip(1) # Flip the time axis so that the most recent time step is last
