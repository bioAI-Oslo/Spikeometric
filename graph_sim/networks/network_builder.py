import torch
import numpy as np
from networks import SpikingNetwork, NumpyNetwork, SparseNetwork
...
class NetworkBuilder:
    ...
    def build_random(self, seed=0, backend='torch'):
        ...
        if backend == 'torch':
            ...
            return NumpyNetwork(W.numpy(), edge_index.numpy(), self.n_neurons, n_networks, threshold=self.threshold, seed=seed)
        elif backend == 'sparse':
            return SparseNetwork(W, edge_index, self.n_neurons, n_networks, threshold=self.threshold, seed=seed)
    ...
    def build_clusters(self, n_clusters, hub_nodes, seed, backend="torch"):
        ...
        if backend == 'torch':
            return TorchNetwork(W, edge_index, self.n_neurons, n_clusters, threshold=self.threshold, seed=seed)
        elif backend == 'numpy':
            return NumpyNetwork(W.numpy(), edge_index.numpy(), self.n_neurons, n_clusters, threshold=self.threshold, seed=seed)
        elif backend == 'sparse':
            return SparseNetwork(W, edge_index, self.n_neurons, n_clusters, threshold=self.threshold, seed=seed)
